#!/usr/bin/env python3
"""
Run XGBoost + Direct Strategy Backtest
3-month validation period - trains separate model per horizon.
"""
import pandas as pd
import numpy as np
...
print("=" * 80)
print("XGBOOST + DIRECT BACKTEST (3-MONTH VALIDATION)")
print("=" * 80)
print()
...
print("Configuration:")
print("  Model: XGBoost")
print("  Strategy: Direct (separate model per horizon)")
...
output_file = output_dir / f'xgboost_direct_3month_{timestamp}.csv'
...
# Compare to XGBoost Recursive
xgb_recursive_w1 = 20.85
xgb_direct_w1 = wapes[0]
difference = xgb_recursive_w1 - xgb_direct_w1

print("Comparison to XGBoost Recursive (3-month):")
print("-" * 80)
print(f"Recursive W1 WAPE:  {xgb_recursive_w1:.2f}%")
print(f"Direct W1 WAPE:     {xgb_direct_w1:.2f}%")
print(f"Difference:         {difference:+.2f} pp {'(Direct better)' if difference > 0 else '(Recursive better)'}")
print()

# Compare to best (LightGBM Direct)
best_w1 = 19.52
difference_from_best = wapes[0] - best_w1

print("Comparison to BEST (LightGBM Direct):")
print("-" * 80)
print(f"LightGBM Direct W1:  {best_w1:.2f}%")
print(f"XGBoost Direct W1:   {xgb_direct_w1:.2f}%")
print(f"Difference:          {difference_from_best:+.2f} pp {'(XGBoost better)' if difference_from_best < 0 else '(LightGBM better)'}")
print()
...
print()
print("=" * 80)
print("XGBOOST DIRECT COMPLETE - ALL BACKTESTS FINISHED")
print("=" * 80)

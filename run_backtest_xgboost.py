...
Single configuration for iterative testing with user approval.
"""
import pandas as pd
import numpy as np
...
print("=" * 80)
print("XGBOOST + RECURSIVE BACKTEST")
print("=" * 80)
print()
...
start_date = pd.Timestamp('2025-03-29')
...
output_file = output_dir / f'xgboost_recursive_{timestamp}.csv'
...
lgb_w1_wape = 19.03
xgb_w1_wape = wapes[0]
improvement = lgb_w1_wape - xgb_w1_wape

print("Comparison to LightGBM:")
print("-" * 80)
print(f"LightGBM W1 WAPE:  {lgb_w1_wape:.2f}%")
print(f"XGBoost W1 WAPE:   {xgb_w1_wape:.2f}%")
print(f"Difference:        {improvement:+.2f} pp {'(XGBoost better)' if improvement > 0 else '(LightGBM better)'}")
print()
